<?php 
	...
	$query = $modal->__select_indi_con("bookmark", [ 
		"1" => ["field" => "user_id",
			   "value" => $data["user_id"], 
			],
		"2"	=> ["field" => "q_code",
	...
	if($rawData->fetch_assoc()['count'] == 0){
		$query = $modal->__insertrec_("bookmark",$data);
		...
		print_r(json_encode($util->__generate_response__(false,"message","Already bookmark")));
	}
?>